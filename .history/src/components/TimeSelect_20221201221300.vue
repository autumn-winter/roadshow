...
export default {
  ...
  methods: {
    dateSelected(date) {
      this.dateSel = true
      Vue.set(this.book_time,0,this.getLocalTime(date))
      Vue.set(this.book_time,1,this.getWeekDay(date))
      console.log(this.dateSel, date,this.getLocalTime(date), this.getWeekDay(date))
    },
    timeSelect(t,index) {
      this.timeSel = true
      if(this.timeSelfSel){
      ...
      Vue.set(this.book_time,2,`${t[0]}-${t[1]}`)
    ...
    timeSelfSelect(t) {
      ...
      Vue.set(this.book_time,2,`${t[0]}:${t[1]}-${t[2]}:${t[3]}`)
      console.log( this.timeSel, this.timeSelfSel, `${t[0]}:${t[1]}-${t[2]}:${t[3]}`)
    },
    nextClick() {
      if(!this.dateSel || !this.timeSel) {
        ...
        console.log(this.$store.book_time)
