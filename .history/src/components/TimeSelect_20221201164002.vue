  ...
  <div class="all">
    <div class="calWrapper">
      ...
      <div class="timeWrapper">
        <div class="timeItemRec" v-for="t,index in times" :class="activeItem==index ? 'activeClass' : '' ">
          <span class="timeFont" @click="timeSelect(t,index)" :style="activeItem==index ? 'color:#FFFFFF' : '' ">{{t[0]}}-{{t[1]}}</span>
        </div>
        <div class="timeSelfRec" :class="activeItem==index ? 'activeClass' : '' ">
          <span class="timeFont" @click="isTimeSelf=true" :style="isTimeSelf ? 'color:#FFFFFF' : '' ">自定义</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    dateSelected(date) {
      this.dateSel = true
      console.log(this.dateSel, date,this.getLocalTime(date))
    },
    timeSelect(t,index) {
      this.timeSel = true
      this.activeItem = index
      console.log( this.timeSel, t,`${t[0]}-${t[1]}`)
    },
    timeSelfSelect(t) {
      this.timeSel = true
      this.isTimeSelf = false
      console.log( this.timeSel, `${t[0]}:${t[1]}-${t[2]}:${t[3]}`)
    },
    nextClick() {
      if(!this.dateSel || !this.timeSel) {
